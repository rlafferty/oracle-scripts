...
set heading on 
set feedback off 
set markup html on spool on HEAD '<TITLE> GE Shared Sourcing Services Database Statistics </TITLE>' 

spool tuning_stats.html  
 
ttitle center 'GE Shared Sourcing Services' skip 1 -
       center 'Database Statistics' skip 1 -
       center 'For SSD4    AS OF: March 23, 2004 11:30 PM' skip 5 -
       left 'LIBRARY CACHE STATISTICS:'  
 
select 'PINS - # of times an item in the library cache was executed ' STAT, to_char(sum(pins)) "STAT VALUE"
from v$librarycache
UNION ALL
select 'RELOADS - # of library cache misses on execution steps' STAT, to_char(sum(reloads)) "STAT VALUE"
from v$librarycache
UNION ALL
Select 'RELOADS / PINS * 100 ' STAT, round((sum(reloads) / sum(pins) *  100),2)||'%' "STAT VALUE"
from    v$librarycache  
/  
...
select lpad(name,20,' ') STAT, value "STAT VALUE", value xv1  
from   v$sysstat  
where  name = 'db block gets'  
/
select lpad(name,20,' ') STAT, value "STAT VALUE", value xv2   
from   v$sysstat  
where  name = 'consistent gets'  
/
select lpad(name,20,' ') STAT, value "STAT VALUE", value xv3   
from   v$sysstat b  
...
column immediate_gets format 999999999 heading 'IMMED GETS'
...
col time_waited format 999,999,999,999 heading 'Time Wait|In Hndrds'  
col total_timeouts format 999,999,999 heading 'Timeout'  
...
column name        format a75                  heading 'Statistic Name'  
column value       format 999,999,999,999,999  heading 'Result'  
...
column pbr       format 999,999,999,999    heading 'Physical|Blk Read'  
column pbw       format 999,999,999,999    heading 'Physical|Blks Wrtn'  
column pyr       format 999,999,999,999    heading 'Physical|Reads'  
column readtim   format 999,999,999        heading 'Read|Time'  
column name      format a40                heading 'DataFile Name'  
column writetim  format 999,999,999        heading 'Write|Time'  
...
ttitle center 'Invalid Objects' skip 2

select owner, object_name "NAME", object_type "TYPE", last_ddl_time "INVALID SINCE"
from dba_objects
...
column segment_type  format a20              heading 'Table/Indx'  
column sum(bytes)    format 999,999,999,999  heading 'Bytes Used'  
column count(*)      format 99,999           heading 'No.'  
 
break on owner skip page 2  
 
ttitle center 'Table Fragmentation Report (200 or More Extents)' skip 2 -  
...
ttitle center 'Index Fragmentation Report (200 or More Extents)' skip 2 -  
  left 'creator: ' owner_var skip 2  
